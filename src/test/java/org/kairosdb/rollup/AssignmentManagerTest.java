...
public class AssignmentManagerTest extends RollupTestBase {
    ...
    public void test_addUnassignedAndRemoveReasignedAndUnassignRemovedTasks()
        ...
        // One server is assigned 1 task and the other 2 tasks
        final List<Long> serverAssignmentCounts = getServerAssignmentCounts(assignmentStore.getAssignments());
        assertThat(serverAssignmentCounts.size(), equalTo(2));
        assertTrue(serverAssignmentCounts.contains(1L));
        assertTrue(serverAssignmentCounts.contains(2L));
    ...
    public void test_score() {
        final RollupTask task1 = new RollupTask("1", new Duration(1, TimeUnit.SECONDS), ImmutableList.of(new Rollup()));
        final RollupTask task2 = new RollupTask("2", new Duration(10, TimeUnit.SECONDS), ImmutableList.of(new Rollup()));
        final RollupTask task3 = new RollupTask("3", new Duration(1, TimeUnit.MINUTES), ImmutableList.of(new Rollup()));
        final RollupTask task4 = new RollupTask("4", new Duration(1, TimeUnit.HOURS), ImmutableList.of(new Rollup()));
        final RollupTask task5 = new RollupTask("5", new Duration(60, TimeUnit.MINUTES), ImmutableList.of(new Rollup()));
        final RollupTask task6 = new RollupTask("6", new Duration(60, TimeUnit.SECONDS), ImmutableList.of(new Rollup()));
        final RollupTask task7 = new RollupTask("7", new Duration(60, TimeUnit.MONTHS), ImmutableList.of(new Rollup()));
        final RollupTask task8 = new RollupTask("8", new Duration(60, TimeUnit.YEARS), ImmutableList.of(new Rollup()));
    ...
    private void setupActiveHosts(final String... hosts) {
        final Date now = new Date();
        final Map<String, ServiceKeyValue> hostMap = new HashMap<>();
    ...
    private String getAssignedHost(final String taskId)
            throws DatastoreException {
        final ServiceKeyValue value = fakeServiceKeyStore.getValue(RollUpAssignmentStoreImpl.SERVICE, RollUpAssignmentStoreImpl.SERVICE_KEY_ASSIGNMENTS, taskId);
        if (value != null) {
    ...
    private List<Long> getServerAssignmentCounts(final Map<String, String> assignments) {
        final SummingMap counts = new SummingMap();
        for (final String server : assignments.values()) {
            counts.put(server, 1L);
        }
        final ArrayList<Long> list = new ArrayList<>(counts.values());
        Collections.sort(list);
        return list;
    }

    private void addStatuses(final RollupTask... tasks)
