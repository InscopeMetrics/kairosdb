...
public class GuiceAggregatorFactoryTest {
    private static void assertProperty(final FeaturePropertyMetadata property, final String name, final String label) {
    ...
    public static void assertProperty(
            final FeaturePropertyMetadata property,
            final String name, final String label, final String description, final String type, final Object defaultValue, final ImmutableList<FeatureValidationMetadata> validations) {
        assertEquals("Names don't match", property.getName(), name);
        assertEquals("Labels don't match", property.getLabel(), label);
    ...
    private static void assertValidations(final ImmutableList<FeatureValidationMetadata> expectedValidations, final ImmutableList<FeatureValidationMetadata> actualValidations) {
        ...
        for (int i = 0; i < actualValidations.size(); i++) {
            final FeatureValidationMetadata expectedValidation = expectedValidations.get(i);
            final FeatureValidationMetadata actualValidation = actualValidations.get(i);
    ...
    public void test_inherited()
        ...
        final AggregatorFactory factory = new AggregatorFactory(injector);

        final ImmutableList<FeatureProcessorMetadata> queryMetadata = factory.getFeatureProcessorMetadata();

        assertEquals(1, queryMetadata.size());
        final FeatureProcessorMetadata metadata = queryMetadata.get(0);
        ...
        final ImmutableList<FeaturePropertyMetadata> properties = metadata.getProperties();
        ...
        final ImmutableList<FeaturePropertyMetadata> samplingProperties = properties.get(7).getProperties();
