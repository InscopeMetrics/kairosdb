...
public class TimeGroupByTest {
    ...
    public void test_getGroupByResults() throws FormatterException {
        final TimeGroupBy groupBy = new TimeGroupBy(new Duration(2, TimeUnit.DAYS), 14);
    ...
    public void test_getGroupId() {
        ...
        final TimeGroupBy groupBy = new TimeGroupBy(new Duration(1, TimeUnit.DAYS), 7);

        // Set start time to be Sunday a week ago
        final long sunday = dayOfWeek(Calendar.SUNDAY);
    ...
    public void test_getGroupId_Month() {
        final Map<String, String> tags = new HashMap<String, String>();
        final TimeGroupBy groupBy = new TimeGroupBy(new Duration(1, TimeUnit.MONTHS), 24);
    ...
    private long dayOfWeek(final int dayOfWeek) {
        final Calendar cal = Calendar.getInstance();
        cal.set(Calendar.DAY_OF_WEEK, dayOfWeek);
        return cal.getTime().getTime();
    }

    private long dayOfMonth(final int year, final int month, final int dayOfMonth) {
        final Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
