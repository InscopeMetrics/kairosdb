...
public class CachedSearchResultTest {
    ...
    public void test_createCachedSearchResult() throws IOException {
        ...
        final String tempFile = System.getProperty("java.io.tmpdir") + "/baseFile";
    ...
    public void test_AddLongsBeyondBufferSize() throws IOException {
        final String tempFile = System.getProperty("java.io.tmpdir") + "/baseFile";
        final CachedSearchResult csResult = CachedSearchResult.createCachedSearchResult(
                "metric2", tempFile, dataPointFactory, true);

        final int numberOfDataPoints = CachedSearchResult.WRITE_BUFFER_SIZE * 2;
        final QueryCallback.DataPointWriter dataPointWriter = csResult.startDataPointSet(LegacyDataPointFactory.DATASTORE_TYPE, Collections.emptySortedMap());
        ...
        final List<DataPointRow> rows = csResult.getRows();
        final DataPointRow taggedDataPoints = rows.iterator().next();

        int count = 0;
        while (taggedDataPoints.hasNext()) {
            final DataPoint dataPoint = taggedDataPoints.next();
    ...
    private void assertValues(final DataPointRow dataPoints, final Number... numbers) {
        int count = 0;
        while (dataPoints.hasNext()) {
            final DataPoint dp = dataPoints.next();

            if (dp.isLong()) {
                final Long value = (Long) numbers[count];
                assertEquals(value.longValue(), dp.getLongValue());
            } else {
                final Double value = (Double) numbers[count];
