...
public class CassandraClientImpl implements CassandraClient, KairosPostConstructInit {
    ...
    @Inject
    public CassandraClientImpl(final ClusterConfiguration configuration, final PeriodicMetrics periodicMetrics) {
    ...
    public void init() {
        ...
        final TokenAwarePolicy readLoadBalancePolicy = new TokenAwarePolicy((m_clusterConfiguration.getLocalDCName() == null) ? new RoundRobinPolicy() : DCAwareRoundRobinPolicy.builder().withLocalDc(m_clusterConfiguration.getLocalDCName()).build(), TokenAwarePolicy.ReplicaOrdering.RANDOM);
        ...
        for (final Map.Entry<String, Integer> hostPort : m_clusterConfiguration.getHostList().entrySet()) {
