...
public class CQLBatch {
    ...
    public CQLBatch(
            ...
            @Named("write_cluster") final ClusterConnection clusterConnection,
    ...
    public void addMetricName(final String metricName) {
        ...
        final BoundStatement bs = new BoundStatement(m_clusterConnection.psStringIndexInsert);
    ...
    private void addBoundStatement(final BoundStatement boundStatement) {
        final Iterator<Host> hosts = m_loadBalancingPolicy.newQueryPlan(m_clusterConnection.getKeyspace(), boundStatement);
        if (hosts.hasNext()) {
    ...
    public void deleteDataPoint(final DataPointsRowKey rowKey, final int columnTime) throws IOException {
        final BoundStatement boundStatement = new BoundStatement(m_clusterConnection.psDataPointsDelete);
    ...
    public void addDataPoint(final DataPointsRowKey rowKey, final int columnTime, final DataPoint dataPoint, final int ttl) throws IOException {
        ...
        final BoundStatement boundStatement = new BoundStatement(m_clusterConnection.psDataPointsInsert);
        boundStatement.setBytesUnsafe(0, DATA_POINTS_ROW_KEY_SERIALIZER.toByteBuffer(rowKey));
        final ByteBuffer b = ByteBuffer.allocate(4);
        b.putInt(columnTime);
        b.rewind();
