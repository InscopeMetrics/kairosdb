...
public class CassandraConfiguration {
    ...
    public CassandraConfiguration(final String keyspaceName) {
    ...
    @Inject
    public void setHostList(@Named(HOST_LIST_PROPERTY) final String hostList) {
        final Iterable<String> strHostList = HostSplitter.split(hostList);
        for (final String hostEntry : strHostList) {
            final Iterator<String> hostPort = PortSplitter.split(hostEntry).iterator();

            final String host = hostPort.next();
            int port = 9042;
