...
public class RollupTaskStatusStoreImpl implements RollupTaskStatusStore {
    ...
    @Inject
    public RollupTaskStatusStoreImpl(final ServiceKeyStore keyStore) {
        this.keyStore = checkNotNull(keyStore, "keystore cannot be null");
    }

    @Override
    public void write(final String id, final RollupTaskStatus status)
        ...
        try {
            ...
            final String json = gson.toJson(status);
    ...
    @Override
    public RollupTaskStatus read(final String id)
            throws RollUpException {
        try {
            checkNotNullOrEmpty(id, "id cannot be null or empty");
            final ServiceKeyValue value = keyStore.getValue(SERVICE, SERVICE_KEY_STATUS, id);
            if (value != null) {
